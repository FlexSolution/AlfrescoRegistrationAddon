...
function getUserGroups() {
    ...
    if (result.status == status.STATUS_OK) {
        return JSON.parse(result.response).groups.map(function (group) {
...
//######################## Check permissions on registred users dashlet ##############################################
                if (uris[0].equals("/com/flex-solution/registered-users")) {
                    var temporaryUserGroups = getUserGroups();
                    var _permission = false;

                    for (var index = 0; index < temporaryUserGroups.length; index++) {
                        if (temporaryUserGroups[index].indexOf("GROUP_ALFRESCO_ADMINISTRATORS") != -1) {
                            _permission = true;
                            break;
                        }
                    }
